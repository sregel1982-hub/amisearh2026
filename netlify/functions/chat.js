...
const ai = new GoogleGenAI({
  apiKey: (typeof Netlify !== "undefined" && Netlify.env.get("GEMINI_API_KEY")) || process.env.GEMINI_API_KEY
});
...
export default async function handler(req) {
  ...
  if (noteId && !notesContext) {
    const { data: noteRow } = await supabase.from("jegyzetek").select("text_content").eq("id", noteId).single();
    if (noteRow?.text_content) notesContext = cleanText(noteRow.text_content);
  }
  ...
  const systemInstruction = "Te egy segítőkész magyar AI tutor vagy. Válaszolj magyarul, érthetően. Ne használj LaTeX parancsokat nyersen. Törteket írj 1 1/2 formában.";
