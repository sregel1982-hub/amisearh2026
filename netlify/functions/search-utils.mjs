...
async function searchArxiv(query) {
  if (!isArxivWorthy(query)) return null;
  try {
...
async function searchCommonsImage(query) {
  try {
    // V4.5 FIX: 3 találatig nézünk, gyorsabb
    const res = await fetch(`https://commons.wikimedia.org/w/api.php?action=query&generator=search&gsrsearch=${encodeURIComponent(query)}&gsrnamespace=6&gsrlimit=3&prop=imageinfo&iiprop=url|extmetadata&iiurlwidth=800&format=json&origin=*`);
    if (!res.ok) return null;
    const data = await res.json();
...
// -------------------------------
// V4.5 FIX - VÉGÉRE BETÉVE, ahogy kérted
// Ez a chat.js V4.5 által várt formában fogad 2 paramétert
// -------------------------------
