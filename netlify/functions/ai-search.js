...
export default async function handler(req) {
      ...
      if (allNotes && allNotes.length > 0) {
        notesContext = allNotes
          .map((n, i) => {
            ...
            const title = n.cim || n.original_name || "ismeretlen";
            return `=== Jegyzet ${i + 1}: ${title} ===\n${clean}`;
          })
          .join("\n\n")
          .substring(0, 100000);
      }
  ...
  const notesInstruction = notesContext
    ? " A felhasználó FELTÖLTÖTT JEGYZETEIT lásd a '=== FELTÖLTÖTT JEGYZETEK ===' részben. Elsősorban ezek alapján válaszolj, és a végén jelöld meg, melyik jegyzetből dolgoztál."
