...
// JAVÍTÁS: export const handler a default helyett
export const handler = async (req) => {
 const user = await getSupabaseUser(req);
 if (!user) return errorRes("Unauthorized", 401);

 const baseUrl = req.url ? new URL(req.url).origin : (getEnv("URL") || "https://amisearh.org");
 ...
 if (req.method === "POST") {
  ...
  triggerIndexDocument(noteId, filePath, baseUrl).catch(console.error);
