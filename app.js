...
// - kép/forrás guard enyhe segéd
// - téma picker + practice PDF/Word letöltés
...
function initThemePicker() {
  ...
  Object.entries(themes).forEach(([key, theme]) => {
    ...
    circle.style.cssText =
      `width:35px;height:35px;border-radius:50%;background:${theme.primary};cursor:pointer;border:3px solid white;`;
...
function applyTheme(name) {
  ...
  st.innerHTML =
    ...
    `.btn-primary,button[type="submit"]{background:${t.primary}!important}` +
...
function guardImagePrompt(q) {
  ...
  if (!wantsHuman) {
    return `${subject}, photorealistic photo, real animal or object, not a book cover, not illustration of a cover, high detail, 4k`;
  }
  return `${subject}, photorealistic, high detail, 4k`;
}

// Csak a practice panel PDF/Word gombjai – NEM generál feladatot
...
// Opcionális: képkérésnél safePrompt jelölés a body-ban (a chat.mjs nem függ tőle, de nem árt)
(function () {
...
function boot() {
  initThemePicker();
  // FONTOS: NE írjuk felül window.generatePractice-t!
  // Az index.html AI-s generatePractice-e marad érvényben.
}
...
console.log('✅ AMISEARCH APP.JS V7 – theme + PDF/Word, generatePractice érintetlen');
